function evalp = pTxParams_torso_phaseI(rawMapsDir)
...
evalp.evalmapsfname = 'torso_maps_phaseI.mat'; % name of the .mat file containing the processed maps
...
evalp.maxInSliceErr = 25; % max in-slice flip angle error (degrees)
evalp.maxInSliceRMSE = evalp.maxInSliceErr/5; % max in-slice flip angle RMS error (degrees)
evalp.maxOutOfSliceErr = ceil(2*evalp.flipAngle*0.01); % max out-of-slice flip angle error (degrees, calculated as 1% of target flip angle, with 2x fudge factor)
evalp.maxOutOfSliceRMSE = evalp.maxOutOfSliceErr/5; % max out-of-slice flip angle RMS error (degrees)
